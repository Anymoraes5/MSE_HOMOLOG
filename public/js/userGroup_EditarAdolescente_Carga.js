...
async function carregarDadosDoUsuario(ID) {
    try {
        ...
        console.log('Todos os campos do formulário foram processados.');
        checkSexo();
        calculaIdade();
        checkMedicamentos();
        checkDemandaSaude();
...
/*fetch(`/editandoPessoasProgramas/${ID}`)
                .then(response => response.json())
                .then(data => {
                    const programasSociais = data.programas_sociais || []; // Adiciona um valor padrão
                    console.log(programasSociais);
    
                    // Limpa o container antes de adicionar novos selects
                    divContainer.innerHTML = '';
    
                    // Preenche os selects com os valores da lista
                    programasSociais.forEach(programa => {
                        criarSelect(programa);
                    });
    
                    // Garantir que ao menos um select seja criado, mesmo que vazio
                    if (programasSociais.length === 0) {
                        criarSelect();
                    }
                }); */
