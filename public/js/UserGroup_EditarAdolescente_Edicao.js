...
function validarEmail(email) {
    ...
    if (!regex.test(email)) {
        alert('Formaro de e-mail inválido');
...
document.getElementById('salvar').addEventListener('click', function() {
    if (!confirm("Tem certeza que deseja cadastrar o usuário?")) {
        ...
        var medicamentos_controlados = document.getElementById('medicamentos_controlados').value;
         ...
         if (!n_processo || n_processo.length < 20) {
         ...
         var cep = validarCEP(document.getElementById('cep'));
         if (!cep) {
        ...
        .then(data => {
            ...
            window.location.href = '/home'; // Redireciona para a página de consulta
        })
        .catch(error => {
